...
function NxMap.GetZoneInfo(mapId)
...
NxMap.BloodelfMapBlks = {	-- Expansion01
...
NxMap.DraeneiMapBlks = {	-- Expansion01
...
NxMap.LIMapBlks = {	-- LostIsles (32,51 max)
	[2426] = "",
	[2427] = "",
	[2428] = "",
	[2429] = "",
...
function NxMap.GetMiniInfo (mapId)

	local winfo
	winfo = RDXMAP.MapWorldInfo[mapId]
	if not winfo then VFL.print(mapId); end
	local id = winfo.MId
	...
	if not id then
		...
		local info = NxMap.MapInfo[id]
		if not info then
			if id then 
				VFL.print("NxMap.GetMiniInfo error id " .. id);
			else
				VFL.print("NxMap.GetMiniInfo error mapid " .. mapId);
			end
			return
		end
	end

	local t = NxMap.MiniMapBlks[id]
...
function NxMap.GetMiniBlkName (miniT, x, y)
