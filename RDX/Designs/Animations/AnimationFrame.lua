...
RDX.RegisterFeature({
	...
	title = VFLI.i18n("Frame: Translation"); category = VFLI.i18n("Animations");
	multiple = true;
...
if ]] .. usecdn .. [[ then 
	...
	UIFromDescriptor = function(desc, parent, state)
		...
		local owner = RDXUI.MakeSlotSelectorDropdown(ui, VFLI.i18n("Animation Group"), state, "AGFra_", nil);
...
RDX.RegisterFeature({
	...
	title = VFLI.i18n("Frame: Rotation"); category = VFLI.i18n("Animations");
	multiple = true;
	IsPossible = function(state)
		if not state:Slot("DesignFrame") then return nil; end
		if not state:Slot("Base") then return nil; end
...
if ]] .. usecdn .. [[ then 
	...
	UIFromDescriptor = function(desc, parent, state)
		...
		local owner = RDXUI.MakeSlotSelectorDropdown(ui, VFLI.i18n("Animation Group"), state, "AGFra_");
		if desc and desc.owner then owner:SetSelection(desc.owner); end
...
RDX.RegisterFeature({
	...
	title = VFLI.i18n("Frame: Scale"); category = VFLI.i18n("Animations");
	multiple = true;
	IsPossible = function(state)
		if not state:Slot("DesignFrame") then return nil; end
		if not state:Slot("Base") then return nil; end
...
RDX.RegisterFeature({
	...
	title = VFLI.i18n("Frame: Alpha"); category = VFLI.i18n("Animations");
	multiple = true;
	IsPossible = function(state)
		if not state:Slot("DesignFrame") then return nil; end
		if not state:Slot("Base") then return nil; end
