...
RDX.RegisterFeature({
	...
	ExposeFeature = function (desc, state, errs)
		...
		-- Verify our frame
		if (not desc.owner) or ((desc.owner ~= "Base") and (not state:Slot("Bkdp_rdx_" .. desc.owner))) then
			VFL.AddError(errs, VFLI.i18n("Owner frame does not exist")); return nil;
		end
		-- Verify color
		if (not desc.color) or (not state:Slot("ColorVar_" .. desc.color)) then
	...
	ApplyFeature = function(desc, state)
		local fvar = RDXUI.ResolveFrameReference(desc.owner);
		local paintCode = [[
VFLUI.ApplyColorBackdropBorderRDX(]] .. fvar .. [[, ]] .. desc.color .. [[);
]];
		state:Attach(state:Slot("EmitPaint"), true, function(code) code:AppendCode(paintCode); end);
	end;
	UIFromDescriptor = function (desc, parent, state)
		...
		-- Owner
		local owner = RDXUI.MakeSlotSelectorDropdown(ui, VFLI.i18n("Owner"), state, "Bkdp_rdx_");
		if desc and desc.owner then owner:SetSelection(desc.owner); end
	...
	CreateDescriptor = function()
		return { feature = "Backdrop Border RDX Colorizer"; };
	end;
});