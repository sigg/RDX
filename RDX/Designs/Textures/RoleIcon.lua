...
frame.]] .. objname .. [[:Hide();
		...
		if desc.test then
			code:AppendCode([[
frame.]] .. objname .. [[._t:SetTexCoord(20/64, 39/64, 1/64, 20/64);
frame.]] .. objname .. [[:Show();
]]);
		else
			code:AppendCode([[
...
if role == "TANK" then
	frame.]] .. objname .. [[._t:SetTexCoord(0, 19/64, 22/64, 41/64);
	if not frame.]] .. objname .. [[:IsShown() then frame.]] .. objname .. [[:Show(0.2); end
elseif role == "HEALER" then
	frame.]] .. objname .. [[._t:SetTexCoord(20/64, 39/64, 1/64, 20/64);
	if not frame.]] .. objname .. [[:IsShown() then frame.]] .. objname .. [[:Show(0.2); end
elseif role == "DAMAGER" then
	frame.]] .. objname .. [[._t:SetTexCoord(20/64, 39/64, 22/64, 41/64);
	if not frame.]] .. objname .. [[:IsShown() then frame.]] .. objname .. [[:Show(0.2); end
else
	if frame.]] .. objname .. [[:IsShown() then frame.]] .. objname .. [[:Hide(0.2); end
		...
		mux:Event_MaskAll("PARTY_MEMBERS_CHANGED", 2);
		return true;
	end;
	UIFromDescriptor = function(desc, parent, state)
		...
		function ui:GetDescriptor()
			...
			return { 
				feature = "tex_role", name = name, owner = owner:GetSelection();
	...
	CreateDescriptor = function()
		return { 
			feature = "tex_role", name = "role", owner = "decor", drawLayer = "ARTWORK";
			w = 14; h = 14;
