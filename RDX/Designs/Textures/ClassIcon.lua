...
RDX.RegisterFeature({
	...
	ExposeFeature = function(desc, state, errs)
		...
		if (not desc.owner) and (not state:Slot("Subframe_" .. desc.owner)) then
...
if UnitIsPlayer(uid) then
	local classii = RDXMD.GetClassIcon(unit:GetClassMnemonic());
	...
	CreateDescriptor = function()
		return { 
			feature = "tex_class", name = "rci", owner = "decor", drawLayer = "ARTWORK";
			w = 14; h = 14;
