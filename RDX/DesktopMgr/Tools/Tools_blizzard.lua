...
local chk_tooltipmouse = VFLUI.Checkbox:new(frameb); chk_tooltipmouse:SetHeight(16); chk_tooltipmouse:SetWidth(200);
...
local lblbkd = VFLUI.MakeLabel(nil, frameb, VFLI.i18n("Bkd"));
lblbkd:SetWidth(34); lblbkd:SetPoint("TOPLEFT", chk_tooltipmouse, "BOTTOMLEFT", 0, -15);
local dd_bkd = VFLUI.MakeBackdropSelectButton(frameb, nil, function() updateGametooltip(); end, nil); 
dd_bkd:SetPoint("LEFT", lblbkd, "RIGHT");
dd_bkd:Show();

local lblfont = VFLUI.MakeLabel(nil, frameb, VFLI.i18n("Fnt"));
lblfont:SetWidth(34); lblfont:SetPoint("TOPLEFT", lblbkd, "BOTTOMLEFT", 0, -15);
local dd_font = VFLUI.MakeFontSelectButton(frameb, nil, function() updateGametooltip(); end, nil); 
dd_font:SetPoint("LEFT", lblfont, "RIGHT");
dd_font:Show();

local lblsb = VFLUI.MakeLabel(nil, frameb, VFLI.i18n("Sb"));
lblsb:SetWidth(34); lblsb:SetPoint("TOPLEFT", lblfont, "BOTTOMLEFT", 0, -15);
local dd_btexture = VFLUI.MakeTextureSelectButton(frameb, nil, function() updateGametooltip(); end, nil); 
dd_btexture:SetPoint("LEFT", lblsb, "RIGHT");
dd_btexture:Show();

local chk_showTarget = VFLUI.Checkbox:new(frameb); chk_showTarget:SetHeight(16); chk_showTarget:SetWidth(200);
...
local chk_showDiffColor = VFLUI.Checkbox:new(frameb); chk_showDiffColor:SetHeight(16); chk_showDiffColor:SetWidth(200);
...
local chk_showTextBar = VFLUI.Checkbox:new(frameb); chk_showTextBar:SetHeight(16); chk_showTextBar:SetWidth(200);
...
local chk_hideInCombat = VFLUI.Checkbox:new(frameb); chk_hideInCombat:SetHeight(16); chk_hideInCombat:SetWidth(200);
...
local chk_showTalent = VFLUI.Checkbox:new(frameb); chk_showTalent:SetHeight(16); chk_showTalent:SetWidth(200);
...
updateGametooltip = function()
	...
	DesktopEvents:Dispatch("DESKTOP_BLIZZARD", desc);
end
